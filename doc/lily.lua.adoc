...
ser reconocidas por Lily.

== `string` _id_
Contiene un identificador para el microprocesador definido. Éste será único en
el sistema, y será usado, entre otros lugares, en la directiva `CPU` de Lily.

== `table` _registros_
La tabla _registros_ contiene todos los registros utilizados por el
microprocesador. Es una tabla asociativa: cada índice corresponde al nombre de
un registro reconocido, y cada valor contiene una tabla que tiene especificadas
...
== `table` _tipos_
La tabla _tipos_ contiene definiciones de tipos personalizados para los
parámetros de un mnemónico. Es una tabla asociativa: cada índice corresponde al
identificador de un tipo, y cada valor contiene una función que acepta un
parámetro &mdash;el valor del parámetro del mnemónico&mdash;, y devuelve un
booleano, indicando si el parámetro es o no del tipo especificado.

El identificador de tipo será una cadena de caracteres que coincida con la
expresión regular en formato PCRE `[a-z0-9_]+`.

Existen 19 identificadores reservados por Lily, que no se pueden utilizar para
identificar un nuevo tipo:

.Tipos predefinidos de Lua
|===
|Tipo |Descripción

|`int3`
|Número entre -2² y 2³-1.

|`uint3`
|Número entre 0 y 2³-1.
...
|`obj`
|Coincide con cualquier objeto.
|===

En los casos en los que hay una función, Lily convertirá los argumentos a tipos
primitivos que puedan leerse fácilmente desde Lua, en vez de pasar las
estructuras originales en C. Así, los argumentos que se puede esperar recibir en
una función pueden ser de uno de los siguientes tipos:

.Conversión de tipos para Lua
|===
|Tipo de argumento|Tipo recibido

|Número
|`integer` con el valor del parámetro

|Objeto
|`string` con el identificador del objeto

|Desplazamiento<...>
|`table`, una lista que contiene los elementos previamente convertidos según lo
...
de esta dupla será una tupla de cadenas, donde cada cadena será un identificador
de tipo (detallado en <<_table_tipos,la sección de la tabla _tipos_>>), y el
segundo, un elemento de cualquiera de los siguientes tipos:
...
dicho mnemónico. Después, se pasará la lista de bytes devuelta a la función para
...
=== Caso
Un caso es un array de dos elementos o partes: un _elemento de coincidencia_,
que Lily usará para ver si hay una coincidencia parcial con la secuencia de
bytes que está buscando (desde el inicio de la secuencia a buscar, o desde el
punto en el que concluyó la búsqueda anterior de esta secuencia), y un _elemento
de resultado_, que sirve para determinar la acción a realizar de haber una
coincidencia.
...
ejecutará. Esta función no devolverá ningún valor, y si lo hace, será ignorado.
