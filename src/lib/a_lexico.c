...
enum lily_error lily_a_lexico(const char* blob, struct lily_lde_lde* simbolos, struct lily_a_lexico_ctx* ctx) {
    ...
    do {
        ...
        if (blob[i] == ';') {
            // Es un comentario
            lily_a_lexico_modo_comentario(blob, &i);
            continue;
        }
        if (blob[i] == '.') {
            // Es una directiva
            ...
            i_inicial = i;
            cod = lily_a_lexico_modo_directiva(blob, &i, &sim);
            if (cod == COD_OK) {
        ...
        if (blob[i] == '$') {
            ...
            i_inicial = i;
            cod = lily_a_lexico_modo_r_etiqueta(blob, &i, &sim);
            if (cod == COD_OK) {
                nodo = lily_lde_insert(simbolos, lily_lde_size(simbolos), (void*) sim);
                if (nodo == NULL) {
                    cod = COD_MALLOC_FALLO;
        ...
        if (blob[i] == '%') {
            ...
            i_inicial = i;
            cod = lily_a_lexico_modo_objeto(blob, &i, &sim);
            if (cod == COD_OK) {
                nodo = lily_lde_insert(simbolos, lily_lde_size(simbolos), (void*) sim);
                if (nodo == NULL) {
                    cod = COD_MALLOC_FALLO;
        ...
        if (blob[i] == 0x27 || blob[i] == 0x22) {
            ...
            i_inicial = i;
            cod = lily_a_lexico_modo_cadena(blob, &i, &sim);
            if (cod == COD_OK) {
                nodo = lily_lde_insert(simbolos, lily_lde_size(simbolos), (void*) sim);
                if (nodo == NULL) {
                    cod = COD_MALLOC_FALLO;
        ...
        if (blob[i] == '0' && (blob[i + 1] == 'x' || blob[i + 1] == 'o' || blob[i + 1] == 'b')) {
            ...
            i_inicial = i;
            i += 2;
            cod = lily_a_lexico_modo_numero(blob, &i, &sim, blob[i-1]);
            if (cod == COD_OK) {
                nodo = lily_lde_insert(simbolos, lily_lde_size(simbolos), (void*) sim);
                if (nodo == NULL) {
                    cod = COD_MALLOC_FALLO;
        ...
        if (isdigit(blob[i]) || blob[i] == '.') {
            ...
            tipo_tentativo = SIMB_NUMERO;
            i_inicial = i;
            cod = lily_a_lexico_modo_numero(blob, &i, &sim, 0);
            if (cod == COD_OK) {
                nodo = lily_lde_insert(simbolos, lily_lde_size(simbolos), (void*) sim);
                if (nodo == NULL) {
                    cod = COD_MALLOC_FALLO;
        ...
        if (lex_esoperador(blob[i])) {
            ...
            i_inicial = i;
            cod = lily_a_lexico_modo_operador(blob, &i, &sim);
            if (cod == COD_OK) {
                nodo = lily_lde_insert(simbolos, lily_lde_size(simbolos), (void*) sim);
                if (nodo == NULL) {
                    cod = COD_MALLOC_FALLO;
        ...
        if (isalpha(blob[i])){
            ...
            i_inicial = i;
            cod = lily_a_lexico_modo_ambiguo(blob, &i, &sim);
            if (cod == COD_OK) {
                nodo = lily_lde_insert(simbolos, lily_lde_size(simbolos), (void*) sim);
                if (nodo == NULL) {
                    cod = COD_MALLOC_FALLO;
